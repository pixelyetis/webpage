...
const yeti = new web3.eth.Contract(yetiABI, yetiAddr)
...
async function mintNFT(){
	try{
		...
		let gasPrice = await web3.eth.getGasPrice() * 2
		let gasEstimate = await yeti.methods.mintYeti(total, amount.toString()).estimateGas({from: senderAddress})
		
		let receipt = await yeti.methods.mintYeti(total, amount.toString()).send({
			from: senderAddress,
			gas: gasEstimate,
			gasPrice: gasPrice
		})
